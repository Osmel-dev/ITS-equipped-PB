% This MATLAB script reproduces the results shown in Fig. 7 of 
...
%% Simulation settings
M = (10:20).^2;                 % num. ITS elements/antennas
N = 4;                          % num. RF chains
...
HPAPowITS = zeros(numel(M),1);
HPAPowHBFC = zeros(numel(M),1);
HPAPowHBPC = zeros(numel(M),1);
HPAPowFD = zeros(numel(M),1);
...
for seed = 1:MCIter
    ...
    HPAPowITS_ = zeros(numel(M),1);
    HPAPowHBFC_ = zeros(numel(M),1);
    HPAPowHBPC_ = zeros(numel(M),1);
    HPAPowFD_ = zeros(numel(M),1);
    for m = 1:numel(M)
        % Positions of the antennas in the array/ITS
        posAnt = positionArray(M(m),dxy);

        % channel coefficients ITS/array -to- device
        h = channelArrayToDevice(posAnt,bgAnt,wavelength,M(m),K,devPos);

        % distance feeder-to-ITS
        da  = 4*dxy*sqrt(M(m)/pi);

        % channel coefficients feeder -to- ITS
        T = channelFeederToITS(posAnt,N,M(m),bgAnt,bgFeeder,rhoITS,wavelength,da);

        % ITS-assisted PB
        [HPAPowITS_(m),~,~] = ITSAssistedPB(h,T,N,p,Pmax,l,etaMax,g,K,M(m),tol);

        % PB equipped with a FC hybrid analog-digital beamforming architecture
        [HPAPowHBFC_(m),~,~] = HBFC(h,N,p,Pmax,l,etaMax,g,K,M(m),tol,RFNetLoss); 
         
        % PB equipped with a PC hybrid analog-digital beamforming architecture
        [HPAPowHBPC_(m),~,~] = HBPC(h,N,p,Pmax,l,etaMax,g,K,M(m),tol,RFNetLoss);
        
        % PB equipped with a FD beamforming architecture
        [HPAPowFD_(m),~] = fullyDigital(h,M(m),p,Pmax,l,etaMax,g,K,tol);
    end
...
plot(M,10*log10(HPAPowITS),'-o','Color','#0072BD','LineWidth',2); hold on
plot(M,10*log10(HPAPowFD),'-.s','Color','#7E2F8E','LineWidth',2);
plot(M,10*log10(HPAPowHBFC),'--^','Color','#A2142F','LineWidth',2);
plot(M,10*log10(HPAPowHBPC),':p','Color','#77AC30','LineWidth',2); hold off
