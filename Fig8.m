% This MATLAB script reproduces the results shown in Fig. 8 of 
...
l = 2:5;                        % l-way parameter Doherty HPA
...
HPAPowITS = zeros(numel(l),1);
HPAPowHBFC = zeros(numel(l),1);
HPAPowHBPC = zeros(numel(l),1);
HPAPowFD = zeros(numel(l),1);
...
for seed = 1:MCIter
    ...
    T = channelFeederToITS(posAnt,N,M,bgAnt,bgFeeder,rhoITS,wavelength,da);

    HPAPowITS_ = zeros(numel(l),1);
    HPAPowHBFC_ = zeros(numel(l),1);
    HPAPowHBPC_ = zeros(numel(l),1);
    HPAPowFD_ = zeros(numel(l),1);
    for ll = 1:numel(l)
        disp(ll)        
         
        % ITS-assisted PB
        [HPAPowITS_(ll),~,~] = ITSAssistedPB(h,T,N,p,Pmax,l(ll),etaMax,g,K,M,tol);
        
        % PB equipped with a FC hybrid analog-digital beamforming architecture
        [HPAPowHBFC_(ll),~,~] = HBFC(h,N,p,Pmax,l(ll),etaMax,g,K,M,tol,RFNetLoss);

        % PB equipped with a PC hybrid analog-digital beamforming architecture
        [HPAPowHBPC_(ll),~,~] = HBPC(h,N,p,Pmax,l(ll),etaMax,g,K,M,tol,RFNetLoss);

        % PB equipped with a FD beamforming architecture
        [HPAPowFD_(ll),~] = fullyDigital(h,M,p,Pmax,l(ll),etaMax,g,K,tol);
    end
...
plot(l,10*log10(HPAPowITS),'-o','Color','#0072BD','LineWidth',2); hold on
plot(l,10*log10(HPAPowFD),'-.s','Color','#7E2F8E','LineWidth',2);
plot(l,10*log10(HPAPowHBFC),'--^','Color','#A2142F','LineWidth',2);
plot(l,10*log10(HPAPowHBPC),':p','Color','#77AC30','LineWidth',2); hold off
